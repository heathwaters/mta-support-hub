...
export async function POST(req: Request) {
  ...
  try {
    // Deviation from plan: URL is not returned to client. CMS sends the link directly
    // to the user's registered email. This prevents clipboard-based account takeover.
    await cmsPost(
      "mtt",
      "AdminMain",
    ...
    console.error(JSON.stringify({ type: "error", ref: requestId, endpoint: "mtt/actions/reset-password", msg: e instanceof Error ? e.message : String(e) }));
    audit.responseStatus = 502;
    audit.durationMs = Date.now() - startTime;
    await logAudit(audit);
    return NextResponse.json({ ok: false, error: "Failed to generate reset link.", code: "CMS_ERROR", ref: requestId }, { status: 502 });
  }
}
