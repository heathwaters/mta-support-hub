...
export async function POST(req: Request) {
  ...
  try {
    await cmsPost("mtt", "AdminMain", "removeLoginAttemptForUser", { usr_id });
    ...
    console.error(JSON.stringify({ type: "error", ref: requestId, endpoint: "mtt/actions/unlock", msg: e instanceof Error ? e.message : String(e) }));
    audit.responseStatus = 502;
    audit.durationMs = Date.now() - startTime;
    await logAudit(audit);
    return NextResponse.json({ ok: false, error: "Failed to unlock account.", code: "CMS_ERROR", ref: requestId }, { status: 502 });
  }
}
